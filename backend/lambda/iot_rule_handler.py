...
def lambda_handler(event, context):
    ...
    try:
        ...
        # Extraer deviceId del topic: orionseye/{deviceId}/data
        device_id = topic.split('/')[1] if '/' in topic else None
        
        if not device_id:
            print("⚠️ No se pudo extraer deviceId del topic")
        ...
        if '/image' in topic:
            ...
            response = lambda_client.invoke(
                FunctionName='process_spectral_image',
            ...
            print(f"Estado del dispositivo: {device_id}")
...
def save_sensor_data(device_id, message):
    """Guarda datos de sensores (temperatura, etc)"""
    # TODO: Implementar si el ESP32 tiene sensores adicionales
